...
supp_paire([],[]).
supp_paire([X],[X]).
supp_paire([X,_|L],[X|R]):-supp_paire(L,R).
...
supp_impaire([],[]).
supp_impaire([_],[]).
supp_impaire([_,Y|L],[Y|R]):-supp_impaire(L,R).
...
taille_list([],0).
taille_list([_|L],T):-taille_list(L,T1),T is T1+1.
%taille([1,2,3,4,5],T).


taille([],0).
taille([_|L],T+1):-taille(L,T).

affich_debut_fin([X]):-format("~1f",[X]),write(" ").
affich_debut_fin([X|L]):-format("~1f",[X]),write(" "),affich_debut_fin(L).

affich_fin_debut([X]):-format("~1f",[X]),write(" ").
affich_fin_debut([X|L]):-affich_fin_debut(L),format("~1f",[X]),write(" ").

existe(X,[X|_]).
existe(X,[_|L]):-existe(X,L).

%max([X],X).
%max([H|L],X):-max(L,X1),X1>H,X is X1.


afficher_F_D([]).
afficher_F_D([X|L]):-afficher_F_D(L),write(X),write(" ").


afficher_D_F([]).
afficher_D_F([X|L]):-write(X),write(" "),afficher_D_F(L).


max([X],X).
max([X|L],R):-max(L,R1),R1 is X,R1>R,R is R1.
...
supprimeTete([],[]).
supprimeTete([_|L],L).
%Supprimer une collone de matrice
supprimeTetemat([],[]).
supprimeTetemat([H|L],[H1|L1]):-
    supprimeTete(H,H1),
    supprimeTetemat(L,L1).
lestete([],[]).
lestete([H|L],[H1|L1]):-tetelist(H,H1),lestete(L,L1).
