...
librer(X):-not(libre(X)),
           sur(Y,X),libre(Y),
           retract(sur(Y,X)),assert(sur(Y,terre)),
           assert(libre(X)).

librer(X):-not(libre(X)),
           sur(Y,X),not(libre(Y)),sur(Z,Y),
           libre(Z),retract(sur(Z,Y)),
           assert(sur(Z,terre)),assert(libre(Y)),
           retract(sur(Y,X)),assert(sur(Y,terre)),
           assert(libre(X)).
...
build(A,B,C):-sur(A,terre),
    ...
    write('on a met ':B),writeln(' sur ':A),nl,
    retract(sur(C,_)),assert(sur(C,B)),nl,
    writeln('on a met ':C),write(' sur ':B),nl.

build(A,B,C):-surterre(A),surterre(B),surterre(C),retract(sur(B,terre)),
    assert(sur(B,A)),
    write('on a met':B),write(' sur ':A),nl,
    retract(sur(C,terre)),
    assert(sur(C,B)),nl,
    write(A:'=> terre '),nl,
